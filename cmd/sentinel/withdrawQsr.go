...
// withdrawQsrCmd withdraws deposited QSR from a sentinel
var withdrawQsrCmd = &cobra.Command{
	Use:   "withdrawQsr",
	Short: "Withdraw deposited QSR",
	Long: `Withdraw QSR that was deposited when registering the sentinel.

When you revoke a sentinel, the deposited QSR becomes available for withdrawal.
...
func init() {
	SentinelCmd.AddCommand(withdrawQsrCmd)
}
...
func runWithdrawQsr(cmdCobra *cobra.Command, args []string) error {
	...
	// Get deposit info
	depositInfo, err := rpcClient.SentinelApi.GetDepositedQsr(parsedAddress)
	if err != nil {
	...
	template := rpcClient.SentinelApi.WithdrawQsr()
